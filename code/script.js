...
//Functions to sort the books alphabetically, by publishing year & highest rating
const sortBooksAtoZ = () => {
...
const allBooks = (filterBooks) => {
  const filterAll = books.filter((bookGenres) => {
    return bookGenres.genre === "All";
  });
  displayBooks(books);
};

const fantasyBooks = (filterBooks) => {
  const filterFantasy = books.filter((bookGenres) => {
    return bookGenres.genre === "Fantasy";
  });
  displayBooks(filterFantasy);
};

const horrorBooks = (filterBooks) => {
  const filterHorror = books.filter((bookGenres) => {
    return bookGenres.genre === "Horror";
  });
  displayBooks(filterHorror);
};

const adventureBooks = (filterBooks) => {
  const filterAdventure = books.filter((bookGenres) => {
    return bookGenres.genre === "Adventure";
  });
  displayBooks(filterAdventure);
};

// Function and eventlistener for a random button generator
randomButton.addEventListener("click", () => {
  const randomBook = Math.floor(Math.random() * 18);
  displayBooks([books[randomBook]]);
});

//Eventlisteners
filterAll.addEventListener("click", allBooks);
filterFantasy.addEventListener("click", fantasyBooks);
filterHorror.addEventListener("click", horrorBooks);
filterAdventure.addEventListener("click", adventureBooks);
...
sortBooksOldest.addEventListener("click", sortedBooksOld);
sortBooksRating.addEventListener("click", highestRating);

//Call on the function to display all the books on the website
displayBooks(books);
