...
function sortLast(booksLastName) {
  ...
  const booksByAuthorLastname = booksLastName
    .slice()
...
function search(searchstring) {
  ...
  for (i = 0; i < books.length; i++) {
    if (
      books[i].description.toLowerCase().includes(searchstring.toLowerCase())
    ) {
      foundWhere = `Found "${searchstring}"in the description: ${books[i].description}.`;
      searchResult.push(books[i]);
    } else if (
      books[i].title.toLowerCase().includes(searchstring.toLowerCase())
    ) {
      foundWhere = `Found "${searchstring}"in the title: ${books[i].title}.`;
      searchResult.push(books[i]);
    } else if (
      books[i].author.toLowerCase().includes(searchstring.toLowerCase())
    ) {
      foundWhere = `Found "${searchstring}"in an authors name: ${books[i].author}`;
      searchResult.push(books[i]);
    } else if (
      books[i].genre.toLowerCase().includes(searchstring.toLowerCase())
    ) {
...
sortAuthorButton.addEventListener("click", () =>
  displayFilteredBooks(booksByAuthor)
);
